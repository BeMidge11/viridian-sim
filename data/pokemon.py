...
class Species:
    ...
    base_stats: dict[str, int] # hp, attack, defense, special-attack, special-defense, speed
    abilities: tuple[str, ...] # List of non-hidden ability slugs
...
def _parse_species(j: dict, dex: int) -> Species | None:
    ...
    # Extract abilities (non-hidden only)
    abilities = []
    for a in j.get("abilities", []):
        if not a.get("is_hidden", False):
            ab_name = a.get("ability", {}).get("name")
            if ab_name:
    ...
    if consolidated_path.exists():
        try:
            ...
            for d in data:
                ...
                if "abilities" in d:
                    d["abilities"] = tuple(d["abilities"])
                # base_stats is a dict, so it's already in the correct format from JSON
